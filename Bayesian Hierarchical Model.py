...
if __name__ == '__main__':
    with pm.Model() as model_mlr:
        alpha_tmp = pm.Normal('alpha_tmp', mu=0, sd=5)
        beta = pm.Normal('beta', mu=0, sd=5, shape=9)

        epsilon = pm.HalfNormal('epsilon', 8)
        ...
        trace_mlr = pm.sample(draws=10000, step=step, start=start, random_seed=123, chain=4, tune=2000)
